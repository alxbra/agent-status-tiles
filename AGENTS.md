...
- The island shows a column only for an active harness: Codex on the left
  ...
  **8 px** dot shows its harness's most important thread: orange when one
  ...
  There is no done state: a finished, failed, or unavailable thread is idle.
  Nothing else: no icons, titles, counts, or badges.
- When a turn finishes, the island plays Cuelume's `success` cue and that
  harness's column pulses green for **10 seconds**, then shows its current
  tone (blue, or hidden when idle). The green ends early for good once the
  harness's tone changes, and a harness waiting for input stays orange
  throughout.
