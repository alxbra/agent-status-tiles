...
- Main opens only a thread in the island's current state whose `canOpen` is set, through the navigator in `docs/navigation.md`: Codex Desktop selects the exact thread; Claude Desktop is activated; a CLI thread activates the terminal its harness recorded (Claude's hook journal host), and Codex CLI threads, which record none, are not opened yet.
- Opening the just-finished thread acknowledges the completion the click saw, only after navigation was dispatched and only if it is still current; a failed or unavailable navigation acknowledges nothing. Unread completions otherwise stay in session state, where they no longer show.
