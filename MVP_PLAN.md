...
- While a tab is extended by the pointer, a reach zone stretching left as far as the widest visible tab and 48 px above and below the stack keeps the dock open, and the pointer's row (each row owns half of its gaps) selects which tab is extended. Leaving the reach zone folds everything. The reach zone only affects hover; clicks land on tab surfaces, and the zone is never published as a native hit region.
